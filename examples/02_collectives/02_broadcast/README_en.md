...
This sample demonstrates how to use the `HcclBroadcast()` API to perform the Broadcast operation. It covers the following functions:
...
- Call `HcclGetRootInfo()` and use `rank 0` as the root rank to generate the rootinfo identifier.
  ...
  - Call `HcclBroadcast()` to broadcast the root node's data to all other ranks in the communicator and display the results.
...
└── broadcast    # Compiled executable file
```
