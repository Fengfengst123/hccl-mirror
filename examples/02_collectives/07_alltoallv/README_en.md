...
This sample demonstrates how to use the `HcclAlltoAllV()` API to perform the AlltoAllV operation. It covers the following functions:
...
- Call `HcclAlltoAllV()` to split the input data into a specific number of blocks along a given dimension, send the blocks sequentially to other ranks, receive data from other ranks, concatenate the received data along the specific dimension in order, and display the result.
