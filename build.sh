...
# 960(960) AIV kernel 编译开关：ENABLE_960="ON"即编, ENABLE_960="OFF"即不编
...
function build_static() { 
     ...
     CUSTOM_OPTION="${CURRENT_CUSTOM_OPTION} -DENABLE_BUILD_DEVICE=ON -DCUSTOM_SIGN_SCRIPT=${CUSTOM_SIGN_SCRIPT} -DENABLE_SIGN=${ENABLE_SIGN}"
     build_device 
     ...
     while IFS= read -r -d '' AIV_O; do 
         ...
         log "Info: Embedded AIV kernel: ${AIV_BASENAME}" 
                  ...
                  \( -name 'hccl_aiv_*_op_910_95.o' -o -name 'hccl_aiv_*_op_960.o' \) -print0) 
...
function usage() {
  ...
  echo "                   Available: scatter,scatter_dpu,scatter_3level,all_reduce,"
  echo "                              all_reduce_parallel,all_reduce_dpu,all_reduce_multilevel,"
  echo "                              all_gather_aicpu,all_gather_dpu,all_gather_v,all_gather_3level,"
  echo "                              dpu_sendrecv,reduce_scatter_aicpu,reduce_scatter_aicpu2,"
  echo "                              reduce_scatter_aicpu3,reduce_scatter_v,reduce_scatter,"
  echo "                              reduce_scatter_3level,reduce_3level,reduce,reduce2,reduce_dpu,"
  echo "                              broadcast_dpu,broadcast,alltoall,alltoallv,alltoallvc"
...
CUSTOM_OPTION="${CUSTOM_OPTION} -DHCCL_ENABLE_960=${ENABLE_960}"
