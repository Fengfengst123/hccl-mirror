...
HcclResult ParseExecTimeout()
{
    ...
    if (execTimeOut > static_cast<double>(UINT32_MAX)) {
        g_algEnvConfig.execTimeOutSet = false;
        g_algEnvConfig.execTimeout = 0;
        HCCL_WARNING("[ParseExecTimeout] HCCL_EXEC_TIMEOUT[%s] is too large, use default.", execTimeOutEnv.c_str());
        return HCCL_E_PARA;
    }
...
HcclResult InitEnvConfig()
{
    ...
    const char* multipleDimensionSplitRatioEnv = std::getenv("HCCL_ALG_MULTIPLE_DIMENSION_SPLIT_RATIO");
    std::string multipleDimensionSplitRatioStr
        = (multipleDimensionSplitRatioEnv != nullptr) ? std::string(multipleDimensionSplitRatioEnv) : "EmptyString";
    RPT_ENV_ERR(
        ret != HCCL_SUCCESS, "EI0001", std::vector<std::string>({"value", "env", "expect"}),
        std::vector<std::string>(
...
HcclResult ParseOpExpansion()
{
    ...
    HCCL_INFO(
        "environmental variable HCCL_OP_EXPANSION_MODE is [%s], aicpuUnfold[%u], aicpuCacheEnable[%u], "
        "aivMode[%u], enableFfts[%u]",
        opExpansionModeEnv.c_str(), g_algEnvConfig.aicpuUnfold, g_algEnvConfig.aicpuCacheEnable, g_algEnvConfig.aivMode,
...
HcclResult ParseRetryEnable()
{
    ...
    if (hcclRetryEnable == "EmptyString") {
        HCCL_INFO("[ParseRetryEnable] HCCL_OP_RETRY_ENABLE is not set. The retryEnable of all levels is set to false.");
        return HCCL_SUCCESS;
    }
    // 去除空格
...
HcclResult ParseInconsistentCheckSwitch(const std::string& inconsistentCheckSwitch)
{
    ...
    HCCL_INFO(
        "[ParseInconsistentCheckSwitch] set by environment to [%s], inconsistentCheckSwitch[%d]",
...
const int32_t& GetInconsistentCheckSwitch() { return g_algEnvConfig.inconsistentCheckSwitch; }
...
bool GetExternalInputHcclAicpuCacheEnable() { return g_algEnvConfig.aicpuCacheEnable > 0; }
...
const u8& GetExternalInputHcclDeterministic() { return g_algEnvConfig.hcclDeterministic; }
