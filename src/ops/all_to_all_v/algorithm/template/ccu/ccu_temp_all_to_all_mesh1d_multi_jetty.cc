...
void CcuTempAllToAllMesh1dMultiJetty::CalcJettySlices(
    uint64_t sliceSize, std::vector<uint64_t>& jettySlice, std::vector<uint64_t>& jettySliceTail)
{
...
std::vector<uint64_t> CcuTempAllToAllMesh1dMultiJetty::BuildTaskArgs(
    uint64_t inputAddr, uint64_t outputAddr, uint64_t token, uint64_t sliceSize, uint64_t srcStride, uint64_t srcOffset,
    uint64_t dstOffset, const std::vector<uint64_t>& goSize, const std::vector<uint64_t>& jettySlice,
...
void CcuTempAllToAllMesh1dMultiJetty::BuildSubmitInfo(
    TemplateResource& templateResource, const std::vector<uint64_t>& taskArgs, uint64_t argSize)
{
    ...
    const OpParam& param, const TemplateDataParams& templateDataParams, TemplateResource& templateResource)
{
    ...
    HCCL_INFO(
        "sliceSize=%llu, totalSliceSize=%llu, srcStride=%llu, srcOffset=%llu, dstOffset=%llu,"
        " dataType_=%lu, dataTypeSize=%lu",
        sliceSize, totalSliceSize, srcStride, srcOffset, dstOffset, dataType_, dataTypeSize);
    ...
    std::vector<uint64_t> taskArgs = BuildTaskArgs(
        inputAddr, outputAddr, token, sliceSize, srcStride, srcOffset, dstOffset, goSize, jettySlice, jettySliceTail);

    HCCL_INFO(
        "[CcuTempAllToAllMesh1dMultiJetty::KernelRun] TaskArgs: inputAddr[%llu], outputAddr[%llu], "
