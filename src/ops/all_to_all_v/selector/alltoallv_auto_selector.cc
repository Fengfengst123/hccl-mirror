    ...
    const std::map<HcclCMDType, std::vector<HcclAlgoType>>& configAlgMap, std::string& selectAlgName) const
{
    ...
    if (topoInfo->topoLevelNums >= TOPO_LEVEL_NUM_3) {
        HCCL_INFO(
            "[AlltoAllVAutoSelector][%s] ccu schedule is not supported when topoLevelNums >= 3(levelNum[%u]), reset to "
            "default.",
    ...
    if (topoInfo->topoLevelNums > 1) {
        if (topoInfo->level0Topo != Level0Shape::CLOS) {
            ...
            if (topoInfo->userRankSize <= CONST_4) {
                selectAlgName = "CcuSchedAllToAllVSoleMesh";
            } else {
        ...
        if (topoInfo->level0Topo == Level0Shape::MESH_1D) {
            if (topoInfo->level0MeshType == Level0MeshType::TWO_DIE_REGULAR) {
                selectAlgName = "CcuSchedAllToAllVSoleMesh2Die";
    ...
    const std::map<HcclCMDType, std::vector<HcclAlgoType>>& configAlgMap, std::string& selectAlgName) const
{
    ...
    if (topoInfo->userRankSize > MAX_RANK_SIZE_V) {
        HCCL_AIV_NOT_MATCH_LOG(
            opParam, HCCL_DEBUG, "[AlltoAllVAutoSelector][%s] rankSize[%u] larger than [%u]", __func__,
    ...
    if (topoInfo->topoLevelNums >= TOPO_LEVEL_NUM_3) {
        HCCL_AIV_NOT_MATCH_LOG(
            opParam, HCCL_DEBUG,
            "[AlltoAllVAutoSelector][%s] aiv is not supported when topoLevelNums >= 3(levelNum[%u]), reset to default.",
            __func__, topoInfo->topoLevelNums);
        return SelectorStatus::NOT_MATCH;
    }
    ...
    const std::map<HcclCMDType, std::vector<HcclAlgoType>>& configAlgMap, std::string& selectAlgName) const
{
    ...
    HCCL_INFO(
        "[AlltoAllVAutoSelector] hccl algo op config: config opType:%d, level0:%u, level1:%u, level2:%u, level3:%u",
