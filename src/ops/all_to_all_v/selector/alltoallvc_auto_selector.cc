    ...
    const std::map<HcclCMDType, std::vector<HcclAlgoType>>& configAlgMap, std::string& selectAlgName) const
{
    ...
    if (topoInfo->level2UbRtp) {
        HCCL_INFO(
            "[AlltoAllVCAutoSelector][%s] ccu schedule is not supported with level2UbRtp, reset to default.", __func__);
        return SelectorStatus::NOT_MATCH;
    }
    if (topoInfo->topoLevelNums >= TOPO_LEVEL_NUM_3) {
        HCCL_INFO(
            "[AlltoAllVCAutoSelector][%s] ccu schedule is not supported when topoLevelNums >= 3(levelNum[%u]), reset "
            "to default.",
    ...
    const std::map<HcclCMDType, std::vector<HcclAlgoType>>& configAlgMap, std::string& selectAlgName) const
{
    ...
    HCCL_INFO(
        "[AlltoAllVCAutoSelector] hccl algo op config: config opType:%d, level0:%u, level1:%u, level2:%u, level3:%u",
    ...
    if (topoInfo->topoLevelNums > 1) {
        if (topoInfo->level0Topo == Level0Shape::MESH_1D) {
            ...
            selectAlgName = "DpuAllToAllVCSoleMesh";
            HCCL_DEBUG(
                "[AlltoAllVCAutoSelector][%s] Level0Shape is CLOS, use algo [%s]", __func__, selectAlgName.c_str());
            return SelectorStatus::MATCH;
        }
    }
