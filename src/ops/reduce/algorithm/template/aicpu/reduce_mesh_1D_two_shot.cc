    ...
    const OpParam& param, const TemplateDataParams& tempAlgParams, TemplateResource& templateResource)
{
    ...
    // 处理数据量为0的场景
    CHK_PRT_RET(
        tempAlgParams.sliceSize == 0, HCCL_INFO("[ReduceMesh1DTwoShot] sliceSize is 0, no need to process"),
        HCCL_SUCCESS);
    ...
    CHK_PRT_RET(
        templateResource.threads.empty(), HCCL_ERROR("[ReduceMesh1DTwoShot] threads is empty"), HCCL_E_INTERNAL);
    ...
    CHK_PRT_RET(
        ...
        HCCL_ERROR(
            "[ReduceMesh1DTwoShot] rank idx[%u] in virtRankMap is invalid, it should be less than rankSize[%u]", myIdx_,
            templateRankSize_),
        HcclResult::HCCL_E_INTERNAL);
...
HcclResult ReduceMesh1DTwoShot::SendRecvDataToPeers(
    ...
    for (u32 remoteIdx = 0; remoteIdx < templateRankSize_; remoteIdx++) {
        ...
        if (remoteIdx == myIdx_) {
            ...
            DataSlice copyDstSlice(
                localHcclBuffPtr, hcclBuffBaseOffset + recvDstSliceOffset, sendSliceSize, sendSliceCount);
            CHK_PRT_RET(
            ...
            void* remoteInBuffPtr = (!enableRemoteMemAccess_) ? sendRecvChannel.remoteCclMem.addr :
                                                                sendRecvChannel.remoteInputGraphMode.addr;
            void* remoteDstBuffPtr = (!enableRemoteMemAccess_) ? sendRecvChannel.remoteCclMem.addr :
                                                                 sendRecvChannel.remoteInputGraphMode.addr;
            void* localDstBuffPtr = (!enableRemoteMemAccess_) ? localHcclBuffPtr : localInBuffPtr;

            u64 sendDstOffset = (!enableRemoteMemAccess_) ? sendDstSliceOffset + hcclBuffBaseOffset :
                                                            sendDstSliceOffset + inBuffBaseOffset;
            u64 recvDstOffset = (!enableRemoteMemAccess_) ? recvDstSliceOffset + hcclBuffBaseOffset :
                                                            recvDstSliceOffset + inBuffBaseOffset;
...
HcclResult ReduceMesh1DTwoShot::DoLocalReduce(
    const TemplateDataParams& tempAlgParam, const OpParam& param, const std::vector<ThreadHandle>& threads)
{
    ...
    void* localBuffPtr
        = (!enableRemoteMemAccess_) ? tempAlgParam.buffInfo.hcclBuff.addr : tempAlgParam.buffInfo.inputPtr;
    u64 localBuffBaseOffset
        = (!enableRemoteMemAccess_) ? tempAlgParam.buffInfo.hcclBuffBaseOff : tempAlgParam.buffInfo.inBuffBaseOff;
    ...
    for (u32 remoteIdx = 0; remoteIdx < templateRankSize_; remoteIdx++) {
        ...
        DataSlice curSrcSlice(
            localBuffPtr, recvSliceSize * remoteIdx + localBuffBaseOffset, recvSliceSize, recvSliceCount);
        CHK_PRT_RET(
            LocalReduce(threads.at(0), curSrcSlice, finalDstSlice, dataType_, reduceOp_),
            HCCL_ERROR("[InsTempReduceMesh1DTwoShot][DoLocalReduce] LocalReduce failed."), HcclResult::HCCL_E_INTERNAL);
    }
    return HCCL_SUCCESS;
}

HcclResult ReduceMesh1DTwoShot::RunReduceScatter(
...
HcclResult ReduceMesh1DTwoShot::GatherLocalData(
    const TemplateDataParams& tempAlgParam, const std::vector<ThreadHandle>& threads) const
{
    ...
    DataSlice copySrcSlice(
        localBuffPtr, localBuffBaseOffset + info.sliceSize * static_cast<u64>(myIdx_), info.sliceSize, info.sliceCount);
    DataSlice copyDstSlice(
        info.localOutBuffPtr, info.outBuffBaseOffset + info.sliceOffset, info.sliceSize, info.sliceCount);
    CHK_PRT_RET(
        LocalCopy(threads.at(myIdx_), copySrcSlice, copyDstSlice),
        HCCL_ERROR("[InsTempReduceMesh1DTwoShot][GatherLocalData] LocalCopy failed."), HcclResult::HCCL_E_INTERNAL);
    return HCCL_SUCCESS;
}

HcclResult ReduceMesh1DTwoShot::GatherRemoteData(
    const TemplateDataParams& tempAlgParam, const std::map<u32, std::vector<ChannelInfo>>& channels,
    const std::vector<ThreadHandle>& threads)
{
    ...
    for (u32 remoteIdx = 0; remoteIdx < templateRankSize_; remoteIdx++) {
        ...
        DataSlice recvDstSlice(
            localOutBuffPtr, outBuffBaseOffset + sliceInfoList_.at(remoteIdx).offset, curSize, curCount);
        const SlicesList recvSlicesList({recvSrcSlice}, {recvDstSlice});
        const DataInfo recvInfo(channel, recvSlicesList);
        CHK_PRT_RET(
            RecvRead(recvInfo, threads.at(remoteIdx)),
            HCCL_ERROR("[InsTempReduceMesh1DTwoShot][GatherRemoteData] RecvRead failed."), HcclResult::HCCL_E_INTERNAL);
    }
    return HCCL_SUCCESS;
}

HcclResult ReduceMesh1DTwoShot::SendToRoot(
    const TemplateDataParams& tempAlgParam, const std::map<u32, std::vector<ChannelInfo>>& channels,
    const std::vector<ThreadHandle>& threads)
{
    LocalSliceInfo info = GetLocalSliceInfo(tempAlgParam);
    ...
    DataSlice sendSrcSlice(
        localBufferPtr, localBuffBaseOffset + info.sliceSize * static_cast<u64>(myIdx_), info.sliceSize,
        info.sliceCount);
    DataSlice sendDstSlice(
        info.localOutBuffPtr, info.outBuffBaseOffset + info.sliceOffset, info.sliceSize,
        info.sliceCount); // SendRead tx 信息不使用
    ...
    CHK_PRT_RET(
        SendRead(sendInfo, threads.at(root_)), HCCL_ERROR("[InsTempReduceMesh1DTwoShot][SendToRoot] SendRead failed."),
        HcclResult::HCCL_E_INTERNAL);
    return HCCL_SUCCESS;
}

HcclResult ReduceMesh1DTwoShot::RunGatherToRoot(
    const TemplateDataParams& tempAlgParam, const std::map<u32, std::vector<ChannelInfo>>& channels,
    const std::vector<ThreadHandle>& threads)
{
    const ThreadHandle& masterThread = threads.at(0);
...
u64 ReduceMesh1DTwoShot::GetThreadNum() const { return templateRankSize_; }
