...
HcclResult InsTempGatherMesh1dIntra::CalcRes(
...
u64 InsTempGatherMesh1dIntra::GetThreadNum() const { return templateRankSize_ > 1 ? templateRankSize_ - 1 : 1; }
...
HcclResult InsTempGatherMesh1dIntra::KernelRun(
...
HcclResult InsTempGatherMesh1dIntra::RunGatherMesh(
    const std::vector<ThreadHandle>& threads, const std::map<u32, std::vector<ChannelInfo>>& channels)
{
    ...
    for (u32 rpt = 0; rpt < tempAlgParams_.repeatNum; ++rpt) {
        ...
        for (u32 threadIdx = 0; threadIdx < subCommRanks_[0].size() - 1; threadIdx++) {
            ...
            HCCL_INFO(
                "[InsTempGatherMesh1dIntra] RunGatherMesh RankIDs[%d], connectedRank[%d], connectedAlgRank[%d].",
            ...
            CHK_PRT_RET(
                ...
                HCCL_ERROR(
                    "[InsTempGatherMesh1dIntra][RankID]=%u threadIdx=%u, threads.size=%u, "
            ...
            if (myAlgSize == 0) { // 发送数据片为0时，只接收数据
                ...
                CHK_PRT_RET(
                    ...
                    HCCL_ERROR("[InsTempGatherMesh1dIntra] [RunGatherMesh] Recv failed."), HcclResult::HCCL_E_INTERNAL);
                ...
                CHK_PRT_RET(
                    ...
                    HCCL_ERROR("[InsTempGatherMesh1dIntra][RunGatherMesh] Send failed."), HcclResult::HCCL_E_INTERNAL);
            } else {
...
void InsTempGatherMesh1dIntra::GetNotifyIdxMainToSub(std::vector<u32>& notifyIdxMianToSub)
{
    ...
    HCCL_INFO(
        "[InsTempGatherMesh1dIntra][GetNotifyIdxMainToSub]threadNum: %u, slaveThreadNum: %u", threadNum,
        slaveThreadNum);
