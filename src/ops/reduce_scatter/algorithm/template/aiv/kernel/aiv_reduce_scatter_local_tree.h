...
private:
    ...
    __aicore__ inline void PublishLocalShard()
    {
        ...
        const uint64_t cclOffset = reinterpret_cast<uint64_t>(myGmIn_) + LocalPublishOffset(blockIdx_);
            ...
            if (blockIdx_ < rankSizeU32_) {
                for (uint32_t offset = blockIdx_; offset < powerOf2; offset += rankSizeU32_) {
                    ...
                    if (backIdx < curBlocks) {
                        ...
                        CpGM2GM(
                            reinterpret_cast<__gm__ T*>(frontOffset), reinterpret_cast<__gm__ T*>(backOffset),
