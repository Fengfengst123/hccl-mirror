...
static CcuResult PairwiseLocalReduce(
    ReduceScatterMesh1DMem2MemContext& ctx, ccu::LocalAddr myOutput, std::vector<ccu::LocalAddr>& inputVec,
    ccu::Variable sliceSize);

static CcuResult
ParseKernelArg(ReduceScatterMesh1DMem2MemContext& ctx, CcuKernelArgReduceScatterMesh1DMem2Mem* kernelArg)
{
...
static CcuResult InitResource(ReduceScatterMesh1DMem2MemContext& ctx)
{
    ...
    for (uint64_t peerId = 0; peerId < arg->rankSize; peerId++) {
        if (peerId == arg->rankId) {
            ...
            HCCL_DEBUG(
                "[CcuKernelReduceScatterMesh1DMem2Mem] MyRank[%u], PeerId[%u], ChannelId[%u]", arg->rankId, peerId,
                channelIdx);
...
static void PreSync(ReduceScatterMesh1DMem2MemContext& ctx)
{
    ...
    for (uint32_t i = 0; i < arg->channelCount; i++) {
        ...
        ccu::WriteVariableWithNotify(
            arg->channels[i], ctx.scratch[arg->rankId], SCRATCH_XN_ID, CKE_IDX_0, 1 << SCRATCH_XN_ID);
        ccu::WriteVariableWithNotify(
...
static void DoReduceScatterRead(ReduceScatterMesh1DMem2MemContext& ctx, uint32_t unrollIdx)
{
    ...
    for (uint32_t rankIdx = 0; rankIdx < arg->rankSize; rankIdx++) {
        ...
        if (rankIdx == arg->rankId) {
            ...
            ccu::Read(
                arg->channels[channelId], ctx.scratchMem[rankIdx], ctx.remoteInput[rankIdx], ctx.sliceSize,
...
static CcuResult DoReduceScatter(ReduceScatterMesh1DMem2MemContext& ctx)
{
    ...
    if (arg->rankSize <= REDUCE_SCATTER_GROUP_REDUCE_MAX_PIECE_CNT) {
        CCU_CHK_RET(ReduceLoopGroup(ctx, myOutput, ctx.myInput, ctx.scratchMem, GetCcuVersion()));
    } else {
...
static CcuResult CreateReduceLoopV1(ReduceScatterMesh1DMem2MemContext& ctx)
{
    ...
    for (int32_t index = 0; index < LOOP_NUM_2; index++) {
        ...
        loops.body[index].reset(new ccu::Func([&ctx, index, bufBase, loopEvt, size, expansionNum, usedBufNum]() {
            ...
            if (size > 1) {
                ccu::LocalReduce(
                    &ctx.moRes.ccuBuf[bufBase], size, ctx.dataType, ctx.outputDataType, ctx.reduceOp,
...
static CcuResult CreateReduceLoopV2(ReduceScatterMesh1DMem2MemContext& ctx)
{
    constexpr uint32_t LOOP_NUM_16 = 16;
    AllocGoResource(ctx.moConfig, ctx.moRes, ctx.resourceAllocated, LOOP_NUM_16, 1, CCU_LOOP_CKE_NUM_REDUCE_LOOP_V2);
    ...
    for (int32_t index = 0; index < LOOP_NUM_2; index++) {
        ...
        loops.body[index].reset(new ccu::Func([&ctx, index, bufBase, e0, e1, e2, size, expansionNum, usedBufNum]() {
            // Step 1: 将数据copy到ccuBuf（区分本rank和其他rank）
            for (uint32_t i = 0; i < size; i++) {
                if (i == ctx.arg->rankId) {
                    // 本rank数据从src copy
                    ccu::LocalCopy(ctx.moRes.ccuBuf[bufBase + i], ctx.loopSrc[index], ctx.loopLen[index], e0, 1 << i);
                } else {
                    // 其他rank数据从scratch copy
                    ccu::LocalCopy(
                        ctx.moRes.ccuBuf[bufBase + i], ctx.loopScratch[index][i], ctx.loopLen[index], e0, 1 << i);
                }
            }
            ...
            if (size > 1) {
                ccu::LocalReduce(
                    &ctx.moRes.ccuBuf[bufBase], size, ctx.dataType, ctx.outputDataType, ctx.reduceOp,
                    ctx.loopLen[index], e1, 1);
                ccu::EventWait(e1, 1);
            }

            // Step 3: Copy结果到dst
            ccu::LocalCopy(ctx.loopDst[index], ctx.moRes.ccuBuf[bufBase], ctx.loopLenExp[index], e2, 1);
            ccu::EventWait(e2, 1);
        }));
    ...
    std::vector<ccu::LocalAddr>& scratchOrg, CcuVersion ccuVersion)
{
    if (ccuVersion == CcuVersion::CCU_V1) {
        ...
        return ReduceLoopGroupV1(ctx, outDstOrg, srcOrg, scratchOrg);
    } else {
        HCCL_INFO("select ReduceLoopGroupV2");
        return ReduceLoopGroupV2(ctx, outDstOrg, srcOrg, scratchOrg);
    }
}
    ...
    std::vector<ccu::LocalAddr>& scratchOrg)
{
    ...
    CCU_CHK_RET(CreateReduceLoopV1(ctx));
    auto& loops = ctx.loopMap["reduceScatterLocalReduce"];
    ...
    std::vector<ccu::LocalAddr>& scratchOrg)
{
    ...
    CCU_CHK_RET(CreateReduceLoopV2(ctx));
    auto& loops = ctx.loopMap["reduceScatterLocalReduce"];
    ...
    CCU_IF(ctx.goSize.loopParam != 0)
    {
        ...
        loops.loopParam[0] = ctx.goSize.loopParam;
    ...
    CCU_IF(ctx.goSize.parallelParam != 0)
    {
        ...
        sliceSizeExpansion = 0;
        for (uint32_t i = 0; i < expansionNum; i++) {
            sliceSizeExpansion += ctx.goSize.residual;
        }
        ...
        ccu::LoopGroup group(ctx.goSize.parallelParam, offsetCfg, xnOffsetCfg, ctx.moConfig.loopCount, grpLoops);
    }
...
CcuResult CcuReduceScatterMesh1DMem2MemKernel(CcuKernelArg arg)
{
    ...
    CCU_IF(ctx.sliceSize != 0) { CCU_CHK_RET(DoRepeatReduceScatter(ctx)); }
