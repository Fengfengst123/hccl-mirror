    ...
    HcclComm comm, TopoInfoWithNetLayerDetails* topoInfo, const char* algName, const OpParam& param)
{
    ...
    if (!MatchTopoForProbe<AlgTopoMatch>(
            topoInfo, algHierarchyInfo, algName, "[InsV2ReduceScatterSoleExecutor][CalcCostCoeff]",
    ...
    if (portNumLevel0.empty()) {
        HCCL_WARNING("[InsV2ReduceScatterSoleExecutor][CalcCostCoeff] portNum is empty");
        return {};
    }
    HCCL_INFO(
        "[InsV2ReduceScatterSoleExecutor][CalcCostCoeff] rankSize=%d, rankSizeLevel0=%d, portNumLevel0=%d, "
        "netTypeLevel0=%d",
        rankSize, rankSizeLevel0, portNumLevel0[0], static_cast<int>(netTypeLevel0));
    ...
    const TopoInfoWithNetLayerDetails* topoInfo, const OpParam& param, const char* algName) const
{
    ...
    AlgHierarchyInfoForAllLevel algHierarchyInfo;
    if (!MatchTopoForProbe<AlgTopoMatch>(
            topoInfo, algHierarchyInfo, algName, "[InsV2ReduceScatterSoleExecutor][GetAlgNetMeta]",
...
REGISTER_ALG_ATTRS(
    ...
    topo.topoCustomCheck = [](const TopoInfoWithNetLayerDetails* topo) -> bool {
        return !topo->level2UbRtp && !topo->topLevelUboe;
    };
