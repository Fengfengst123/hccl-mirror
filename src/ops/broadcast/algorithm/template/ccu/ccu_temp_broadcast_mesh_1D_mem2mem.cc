...
std::vector<CostModelParam> CcuTempBroadcastMesh1DMem2Mem::CalcCostCoeff(CalcCostCoeffParam param)
{
    ...
    // CLOS 链路框间带宽争用修正：框间 rank 越多，实际带宽争用越严重，预估偏低越多
...
HcclResult CcuTempBroadcastMesh1DMem2Mem::CalcRes(
    ...
    const OpParam& param, const TemplateDataParams& templateDataParams, TemplateResource& templateResource)
{
    ...
    HCCL_INFO(
        "[CcuTempBroadcastMesh1DMem2Mem::KernelRun] TaskArgs: inputAddr[%llu], outputAddr[%llu], "
    ...
    CHK_RET(FillCachedArgs(
        submitInfo, inputAddr, outputAddr, token, currentRankSliceInputOffset, currentRankSliceOutputOffset,
...
void CcuTempBroadcastMesh1DMem2Mem::SetRoot(u32 root)
{
    ...
    HCCL_INFO(
        "[CcuTempBroadcastMesh1DMem2Mem][SetRoot] myRank_ [%u], set root_ [%u] subCommRanks[%u]", myRank_, root,
        subCommRootId_);
}

u64 CcuTempBroadcastMesh1DMem2Mem::GetThreadNum() const { return 1; }
