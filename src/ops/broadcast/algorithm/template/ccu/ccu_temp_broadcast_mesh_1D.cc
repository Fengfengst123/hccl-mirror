...
HcclResult CcuTempBroadcastMesh1D::CalcRes(
    ...
    AlgResourceRequest& resourceRequest)
{
    ...
    HCCL_DEBUG(
        "[CcuTempBroadcastMesh1D::CalcRes] notifyNumOnMainThread[%u] slaveThreadNum[%u]",
    ...
    HCCL_DEBUG(
        "[CcuTempBroadcastMesh1D::CalcRes] channelDescs.size()=%llu, dimsize=%llu, "
...
HcclResult CcuTempBroadcastMesh1D::KernelRun(
    const OpParam& param, const TemplateDataParams& templateDataParams, TemplateResource& templateResource)
{
    ...
    HCCL_INFO(
        "[CcuTempBroadcastMesh1D::KernelRun] TaskArgs: inputAddr[%llu], outputAddr[%llu], "
