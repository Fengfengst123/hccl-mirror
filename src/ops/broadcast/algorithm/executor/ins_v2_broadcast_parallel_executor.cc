...
InsBroadcastParallelExecutor<
    AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1, InsAlgTemplate2, InsAlgTemplate3>::InsBroadcastParallelExecutor()
{}
...
template <
    ...
    typename InsAlgTemplate3>
HcclResult
InsBroadcastParallelExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1, InsAlgTemplate2, InsAlgTemplate3>::CalcRes(
    ...
    const AlgHierarchyInfoForAllLevel& algHierarchyInfo, AlgResourceRequest& resourceRequest)
{
    ...
    std::shared_ptr<InsAlgTemplate2> algTemplate2
        = std::make_shared<InsAlgTemplate2>(param, topoInfo->userRank, temp0HierarchyInfo);
    std::shared_ptr<InsAlgTemplate3> algTemplate3
        = std::make_shared<InsAlgTemplate3>(param, topoInfo->userRank, temp1HierarchyInfo);
    ...
    if (param.engine == COMM_ENGINE_CCU) {
        ...
        HCCL_INFO(
            "[CalcRes] localRankSize: myRank[%d] intraLocalRankSize[%u] interLocalRankSize[%u] rankSize_[%u]", myRank_,
    ...
    if (param.engine != COMM_ENGINE_CCU) {
        ...
        // ccu scatter
        HCCL_INFO(
            "[InsBroadcastParallelExecutor][CalcRes] intraTemplate has [%d] kernels.",
        ...
        HCCL_INFO(
            "[InsBroadcastParallelExecutor][CalcRes] interTemplate has [%d] kernels.",
        ...
        // // ccu allgather
        HCCL_INFO(
            "[InsBroadcastParallelExecutor][CalcRes] intraTemplate1 has [%d] kernels.",
            intraTempRequest.ccuKernelNum[0]);
        resourceRequest.ccuKernelInfos.insert(
            resourceRequest.ccuKernelInfos.end(), intraTempRequest0.ccuKernelInfos.begin(),
            intraTempRequest0.ccuKernelInfos.end());
        resourceRequest.ccuKernelNum.emplace_back(intraTempRequest0.ccuKernelNum[0]);
        HCCL_INFO(
            "[InsBroadcastParallelExecutor][CalcRes] interTemplate1 has [%d] kernels.",
            interTempRequest.ccuKernelNum[0]);
        resourceRequest.ccuKernelInfos.insert(
    ...
    for (auto i = 0; i < resourceRequest.notifyNumPerThread.size(); i++) {
        HCCL_DEBUG(
            "[InsBroadcastParallelExecutor][CalcRes] myRank[%u], notifyNumPerThread[%u]=[%u]", myRank_, i,
    ...
    Orchestrate(const OpParam& param, const AlgResourceCtxSerializable& resCtx)
{
    ...
    CHK_PRT_RET(
        ret != HCCL_SUCCESS,
        HCCL_ERROR(
            "[InsBroadcastParallelExecutor][Orchestrate]errNo[0x%016llx] Broadcast executor kernel run failed",
            HCCL_ERROR_CODE(ret)),
        ret);
    return HcclResult::HCCL_SUCCESS;
}
    ...
    CalcCostCoeff(HcclComm comm, TopoInfoWithNetLayerDetails* topoInfo, const char* algName, const OpParam& param)
{
    ...
    auto p2 = InsAlgTemplate1::CalcCostCoeff(CalcCostCoeffParam{
        rankSizeLevel1, ratio / rankSizeLevel1 / rankSizeLevel0, netTypeLevel1, BufferType::HCCL_BUFFER,
    ...
    auto p3 = InsAlgTemplate0::CalcCostCoeff(CalcCostCoeffParam{
        rankSizeLevel0, (1.0f - ratio) / rankSizeLevel0 / rankSizeLevel1, netTypeLevel0, BufferType::HCCL_BUFFER,
    ...
    GetAlgNetMeta(const TopoInfoWithNetLayerDetails* topoInfo, const OpParam& param, const char* algName) const
{
    ...
    meta.dataRatios
        = {ratio / rankSizeLevel0,                        // seg0: L0 scatter part0
           (1 - ratio) / rankSizeLevel1,                  // seg1: L1 scatter part1
           ratio / rankSizeLevel1 / rankSizeLevel0,       // seg2: L1 scatter part0
           (1 - ratio) / rankSizeLevel0 / rankSizeLevel1, // seg3: L0 scatter part1
           ratio / rankSizeLevel1 / rankSizeLevel0,       // seg4: L1 allgather part0
           (1 - ratio) / rankSizeLevel0 / rankSizeLevel1, // seg5: L0 allgather part1
           ratio / rankSizeLevel0,                        // seg6: L0 allgather part0
    ...
    GetParallelDataSplit(std::vector<float>& splitDataSize, const AlgResourceCtxSerializable& resCtx) const
{
    ...
    HCCL_INFO(
        "[InsBroadcastParallelExecutor] meshFirstRatio[%f], closFirstRatio[%f]", splitDataSize[0], splitDataSize[1]);
    return;
}
...
uint64_t
InsBroadcastParallelExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1, InsAlgTemplate2, InsAlgTemplate3>::
    GetRankSize(const std::vector<std::vector<u32>>& vTopo) const
{
    uint64_t count = 1;
...
template <
    typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1, typename InsAlgTemplate2,
    typename InsAlgTemplate3>
HcclResult InsBroadcastParallelExecutor<
    AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1, InsAlgTemplate2, InsAlgTemplate3>::CalcLocalRoot()
{
    CHK_PRT_RET(
    ...
    HCCL_DEBUG(
        "[DEBUG] new myrank[%u], rankIdxLevel0_[%u] ,rankIdxLevel1_[%u] interLocalRoot_[%u] intraLocalRoot_[%u]",
    ...
    GenDataParamsAllRank(const u64 sliceCount, const u32 LocalRankSize, TemplateDataParams& dataParams) const
{
    ...
    HCCL_INFO(
        "[InsBroadcastParallelExecutor][GenDataParamsAllRank] tailSize[%llu] sliceSize[%llu] count[%llu] "
        "inputSliceStride[%llu] outputSliceStride[%llu]",
        dataParams.tailSize, dataParams.sliceSize, dataParams.count, dataParams.inputSliceStride,
        dataParams.outputSliceStride);
    return;
}
        ...
        InsAlgTemplate1& tempAlgInter, InsAlgTemplate2& tempAlgIntra1, InsAlgTemplate3& tempAlgInter1)
{
    ...
    HCCL_DEBUG(
        "[InsBroadcastParallelExecutor][OrchestrateLoop] myRank_[%d],scratchOffsetCountInterStage0[%d], "
        "scratchOffsetCountIntraStage1[%d] loopTimes[%u]",
    ...
    while (processedCount < dataCount_) {
        ...
        CHK_PRT_RET(
            ...
            HcclResult::HCCL_E_INTERNAL);
#ifndef AICPU_COMPILE
        const bool saveCcuFastLaunchCtx
            = loopTimes == 1 && loopIndex == 0 && processedCount == 0 && currProcessedCount == dataCount_
              && param.engine == CommEngine::COMM_ENGINE_CCU && param.opMode != OpMode::OFFLOAD;
#endif
        ...
        HCCL_DEBUG(
            "[InsBroadcastParallelExecutor][OrchestrateLoop] RunTemplateInter1 myRank_[%d], dataOffset0[%d], "
            "currCountPart0[%d], loopTimes[%d]",
            myRank_, dataOffset0, currCountPart0, loopTimes);
        HCCL_DEBUG(
            "[InsBroadcastParallelExecutor][OrchestrateLoop] RunTemplateInter1 myRank_[%d], dataOffset0[%d], "
            "currCountPart0[%d], scratchOffsetCountInterStage0[%d] "
        ...
        const TemplateResource& intraTempAlgRes1, const TemplateResource& interTempAlgRes1, u32 notifyNumOnMainThread)
{
    ...
    u32 ccuKernelNum = ccuKernelLaunchNumIntra1_ + ccuKernelLaunchNumInter0_ + ccuKernelLaunchNumIntra0_
                       + ccuKernelLaunchNumInter1_ + ccuKernelLaunchNumIntra11_ + ccuKernelLaunchNumInter01_
                       + ccuKernelLaunchNumIntra01_ + ccuKernelLaunchNumInter11_;
    if (ccuKernelNum < 1) {
        HCCL_INFO("[InsBroadcastParallelExecutor][FastLaunchSaveCtx] ccu kernel num is 0, no need to save.");
        return HCCL_SUCCESS;
    }
    HCCL_INFO(
        "[InsBroadcastParallelExecutor][FastLaunchSaveCtx] threadNum[%llu], ccuKernelNum[%llu]", threadNum,
        ccuKernelNum);
        ...
        InsAlgTemplate0& tempAlgIntra) const
{
    ...
    if (intraLocalRoot_ == root_ && currCountPart > 0) {
        // 数据0的server内的mesh算法
        HCCL_INFO(
            "[InsBroadcastParallelExecutor][RunTemplateIntra0] myRank_[%u] intraLocalRoot[%u] currCountPart[%u]",
        ...
        GenDataParamstempAlg(
            param, resCtx, dataOffset, currCountPart, scratchOffsetCount, dataParams, intraLocalRankSize_);
        ...
        InsAlgTemplate1& tempAlgInter)
{
    ...
    if (interLocalRoot_ == root_ && currCountPart > 0) {
        // 数据1的server间的nhr算法
        HCCL_INFO(
            "[InsBroadcastParallelExecutor][RunTemplateInter1] myRank_[%u] interLocalRoot[%u] currCountPart[%u]",
            myRank_, interLocalRoot_, currCountPart);
        GenDataParamsBufferType(BufferType::INPUT, BufferType::HCCL_BUFFER, BufferType::HCCL_BUFFER, dataParams);
        GenDataParamstempAlg(
            param, resCtx, dataOffset, currCountPart, scratchOffsetCount, dataParams, interLocalRankSize_);
        ...
        InsAlgTemplate1& tempAlgInter)
{
    ...
    if (currCountPart0_ > 0) {
        // 数据0的server间的nhr算法
        HCCL_DEBUG(
            "[InsBroadcastParallelExecutor][RunTemplateInter0] myRank_[%d], dataOffset[%d], currCountPart[%d], "
            "scratchOffsetCountInterStage[%d]",
...
HcclResult
InsBroadcastParallelExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1, InsAlgTemplate2, InsAlgTemplate3>::
    FastLaunchTemplateInter0(
        const OpParam& param, const u32 kernelNum, TemplateFastLaunchCtx& tempFastLaunchCtxInter,
        InsAlgTemplate1& tempAlgInter) const
{
    if (kernelNum > 0) {
        ...
        InsAlgTemplate0& tempAlgIntra)
{
    ...
    if (currCountPart1_ > 0) {
        // 数据1的server内的mesh算法
        HCCL_DEBUG(
            "[InsBroadcastParallelExecutor][RunTemplateIntra1] myRank_[%d], dataOffset[%d], currCountPart[%d], "
            "scratchOffsetCountInterStage[%d]",
        ...
        InsAlgTemplate3& tempAlgInter1)
{
    ...
    if (currCountPart0_ > 0) {
        // 数据0的server间的nhr算法
        HCCL_DEBUG(
            "[InsBroadcastParallelExecutor][RunTemplateInter01] myRank_[%d], dataOffset[%d], currCountPart[%d], "
            "scratchOffsetCountInterStage[%d]",
            myRank_, dataOffset0Inter_, currCountPart0_, scratchOffsetCountInterStage1_);
        GenDataParamsBufferType(BufferType::HCCL_BUFFER, BufferType::HCCL_BUFFER, BufferType::HCCL_BUFFER, dataParams);
        GenDataParamstempAlg(
        ...
        InsAlgTemplate2& tempAlgIntra1)
{
    ...
    if (currCountPart1_ > 0) {
        // 数据1的server内的mesh算法
        HCCL_DEBUG(
            "[InsBroadcastParallelExecutor][RunTemplateIntra11] myRank_[%d], dataOffset[%d], currCountPart[%d], "
            "scratchOffsetCountInterStage[%d]",
            myRank_, dataOffset0Intra_, currCountPart1_, scratchOffsetCountIntraStage1_);
        GenDataParamsBufferType(BufferType::HCCL_BUFFER, BufferType::HCCL_BUFFER, BufferType::HCCL_BUFFER, dataParams);
        GenDataParamstempAlg(
        ...
        InsAlgTemplate2& tempAlgIntra1)
{
    if (currCountPart > 0) {
        ...
        GenDataParamstempAlg(
            param, resCtx, dataOffset, currCountPart, scratchOffsetCount, dataParams, intraLocalRankSize_);
        HCCL_DEBUG(
            "[InsBroadcastParallelExecutor][RunTemplateIntra01] dataOffset[%d], myRank_[%d], inBuffBaseOff[%d], "
            "outBuffBaseOff[%d], hcclBuffBaseOff[%d]",
            dataOffset, myRank_, dataParams.buffInfo.inBuffBaseOff, dataParams.buffInfo.outBuffBaseOff,
            dataParams.buffInfo.hcclBuffBaseOff);
        CHK_RET(tempAlgIntra1.KernelRun(param, dataParams, templateResource));
    }
    return HcclResult::HCCL_SUCCESS;
        ...
        InsAlgTemplate3& tempAlgInter1) const
{
    if (currCountPart > 0) {
        // 数据1的server间的nhr算法
        HCCL_DEBUG(
            "[InsBroadcastParallelExecutor][RunTemplateInter11] myRank_[%d], dataOffset[%d], currCountPart[%d], "
            "scratchOffsetCountInterStage[%d]",
...
REGISTER_EXECUTOR_BY_FOUR_TEMPS(
    HcclCMDType::HCCL_CMD_BROADCAST, CcuSchedBroadcastParallelMeshNHR, InsBroadcastParallelExecutor, TopoMatchTwoLevel,
...
REGISTER_ALG_ATTRS(
    CcuSchedBroadcastParallelMeshNHRMultiJetty, topo.maxSupportRankSize = CCU_SCHED_MAX_RANK_SIZE;
