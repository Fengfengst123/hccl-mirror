...
InsV2BroadcastConcurrentExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1>::CalcCostCoeff(
    HcclComm comm, TopoInfoWithNetLayerDetails* topoInfo, const char* algName, const OpParam& param)
{
    (void)comm;
    AlgHierarchyInfoForAllLevel algHierarchyInfo;
    if (!MatchTopoForProbe<AlgTopoMatch>(
            topoInfo, algHierarchyInfo, algName, "[InsV2BroadcastConcurrentExecutor][CalcCostCoeff]",
    ...
    if (portNumLevel0.empty() || portNumLevel1.empty()) {
        HCCL_WARNING("[InsV2BroadcastConcurrentExecutor][CalcCostCoeff] portNum is empty");
        return {};
    }

    HCCL_INFO(
        "[InsV2BroadcastConcurrentExecutor][CalcCostCoeff] rankSize=%d, netTypeLevel0=%d, netTypeLevel1=%d", rankSize,
    ...
    const TopoInfoWithNetLayerDetails* topoInfo, const OpParam& param, const char* algName) const
{
    AlgHierarchyInfoForAllLevel algHierarchyInfo;
    if (!MatchTopoForProbe<AlgTopoMatch>(
            topoInfo, algHierarchyInfo, algName, "[InsV2BroadcastConcurrentExecutor][GetAlgNetMeta]",
    ...
    const AlgHierarchyInfoForAllLevel& algHierarchyInfo, AlgResourceRequest& resourceRequest)
{
    ...
    CHK_PRT_RET(
        channelDescs0.empty(),
        HCCL_ERROR(
            "[InsV2BroadcastConcurrentExecutor][%s] channelDescs0.size()[%zu] is zero.", __func__,
            channelDescs0.size()),
    ...
    CHK_PRT_RET(
        channelDescs1.empty(),
        HCCL_ERROR(
            "[InsV2BroadcastConcurrentExecutor][%s] channelDescs1.size()[%zu] is zero.", __func__,
            channelDescs1.size()),
        HcclResult::HCCL_E_INTERNAL);
    ...
    CHK_PRT_RET(
        ...
        HCCL_ERROR(
            "[InsV2BroadcastConcurrentExecutor][%s] channelDescs0.size()[%zu] is not equal to "
            "channelDescs1.size()[%zu]",
    ...
    const OpParam& param, const AlgResourceCtxSerializable& resCtx)
{
    ...
    if (algHierarchyInfo_.infos.empty() || algHierarchyInfo_.infos[0].size() < 2) {
        HCCL_ERROR(
            "[InsV2BroadcastConcurrentExecutor][%s] algHierarchyInfo_.infos[0] is invalid (empty or size < 2).",
            __func__);
        return HCCL_E_PARA;
    }
    ...
    CHK_PRT_RET(
        ...
        HCCL_ERROR(
            "[InsV2BroadcastConcurrentExecutor][%s] maxCountPerLoopforTemp is 0, maxCount0[%llu], maxCount1[%llu], "
            "dataTypeSize_[%llu].",
    ...
    HCCL_INFO(
        "[InsV2BroadcastConcurrentExecutor][%s] portNum0[%llu], portNum1[%llu], dataCount[%llu], "
    ...
    for (u32 loopIndex = 0; loopIndex < loopTimesforTemp0 || loopIndex < loopTimesforTemp1; loopIndex++) {
        HCCL_INFO(
            "[InsV2BroadcastConcurrentExecutor][%s] loopIndex[%u], loopTimesforTemp0[%u], loopTimesforTemp1[%u]",
...
template <typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1>
void InsV2BroadcastConcurrentExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1>::GetParallelDataSplit(
    const OpParam& param, std::vector<float>& splitDataSize) const
{
    ...
    HCCL_INFO(
        "[InsV2BroadcastConcurrentExecutor][GetParallelDataSplit] portNum0[%u], portNum1[%u], splitData[%.4f]",
...
REGISTER_ALG_ATTRS(
    AicpuBroadcastConcurMeshNHR, topo.maxTopoLevelNum = 1; topo.supportLevel0Topos = LEVEL0_TOPO_MESH_1D_CLOS;
...
REGISTER_ALG_ATTRS(
    CcuMSBroadcastConcurMeshNHR, topo.maxTopoLevelNum = 1; topo.supportLevel0Topos = LEVEL0_TOPO_MESH_1D_CLOS;
    topo.topoCustomCheck = [](const TopoInfoWithNetLayerDetails* topo) -> bool {
        return AutoSelectorBase::IsLayerAllConnetedWithTopo(topo, 0, CommTopo::COMM_TOPO_1DMESH);
    });
#endif
