...
public:
    ...
    virtual HcclResult Prepare(
        ...
        ThreadHandle thread, const HcclReduceOp reductionOp = HCCL_REDUCE_RESERVED,
    ...
    u32 DataUnitSize(HcclDataType dataType) const
    {
        if (dataType >= HCCL_DATA_TYPE_RESERVED) {
            HCCL_ERROR(
                ...
                GetDataTypeEnumStr(dataType).c_str(), HCCL_DATA_TYPE_INT8,
...
protected:
    ...
    HcclResult
    ExecuteBarrier(ChannelInfo& preChannel, ChannelInfo& aftChannel, u32 notifyIdx, ThreadHandle thread) const;
