    ...
    AlgResourceRequest& resourceRequest)
{
    ...
    HCCL_DEBUG(
        "[CcuTempAllGatherNHR1DMultiJettyMem2Mem::CalcRes] notifyNumOnMainThread[%u] slaveThreadNum[%u]",
    ...
    CHK_SAFETY_FUNC_RET(strcpy_s(
        kernelInfo.kernelFuncName, sizeof(kernelInfo.kernelFuncName), "CcuAllGatherNHR1DMultiJettyMem2MemKernel"));
...
HcclResult CcuTempAllGatherNHR1DMultiJettyMem2Mem::PrepareLaunchArgs(
    ...
    uint64_t& argSize)
{
    ...
    uint64_t sliceCountPerJetty
        = dataCount / jettyNum_ / (HCCL_MIN_SLICE_ALIGN / dataTypeSize) * (HCCL_MIN_SLICE_ALIGN / dataTypeSize);
    ...
    HCCL_DEBUG(
        "[CcuTempAllGatherNHR1DMultiJettyMem2Mem] inputAddr[%llu], outputAddr[%llu],"
...
HcclResult CcuTempAllGatherNHR1DMultiJettyMem2Mem::KernelRun(
    const OpParam& param, const TemplateDataParams& templateDataParams, TemplateResource& templateResource)
{
    ...
    CHK_RET(FillCachedArgs(
        ...
        taskArgs[7], taskArgs[8], taskArgs[9], taskArgs[10], taskArgs[11], taskArgs[12], taskArgs[13], taskArgs[14],
        taskArgs[15], buffInfo_.inBuffBaseOff, buffInfo_.outBuffBaseOff, mySubCommRank_));
...
HcclResult
CcuTempAllGatherNHR1DMultiJettyMem2Mem::FastLaunch(const OpParam& param, const TemplateFastLaunchCtx& tempFastLaunchCtx)
{
    (void)param;
    ...
    if (launchRet != CCU_SUCCESS) {
        HCCL_ERROR(
            "[CcuTempAllGatherNHR1DMultiJettyMem2Mem::FastLaunch] kernel launch failed, ccuRet -> %d", launchRet);
        return ConvertCcuToHccl(launchRet);
    }
...
u64 CcuTempAllGatherNHR1DMultiJettyMem2Mem::GetThreadNum() const { return 1; }
