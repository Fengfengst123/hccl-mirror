...
HcclResult CcuTempAllGatherOmniPipeMesh1DMem2Mem::CalcRes(
    ...
    AlgResourceRequest& resourceRequest)
{
    ...
    HCCL_DEBUG(
        "[CcuTempAllGatherOmniPipeMesh1DMem2Mem][%s] notifyNumOnMainThread[%u] slaveThreadNum[%u]", __func__,
        resourceRequest.notifyNumOnMainThread, resourceRequest.slaveThreadNum);
    // 创建每个kernel的ctxArg，放入kernelInfo, 然后将kernelinfo放入resourceRequest.ccuKernelInfos
    CcuKernelInfo kernelInfo;
    CHK_SAFETY_FUNC_RET(strcpy_s(
        kernelInfo.kernelFuncName, sizeof(kernelInfo.kernelFuncName), "CcuAllGatherOmniPipeMesh1DMem2MemKernel"));
...
u64 CcuTempAllGatherOmniPipeMesh1DMem2Mem::GetThreadNum() const { return 1; }
    ...
    if (localCopyFlag == 1) {
        ...
        for (uint32_t rpt = 0; rpt < repeatNum; ++rpt) {
            ...
            HCCL_DEBUG(
                ...
                "inBuffBaseOff[%llu] "
                "outBuffBaseOff[%llu] inputAddr[%llu] outputAddr[%llu] sliceSize[%llu] sliceStride[%llu] "
                "localCopyFlag[%llu]",
