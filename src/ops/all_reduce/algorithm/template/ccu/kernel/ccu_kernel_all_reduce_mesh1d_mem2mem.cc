...
static CcuResult ParseKernelArg(AllReduceMeshMem2Mem1DContext& ctx, CcuKernelArgAllReduceMeshMem2Mem1D* kernelArg)
{
    ...
    if (ctx.outputDataType == HcclDataType::HCCL_DATA_TYPE_RESERVED) {
        ...
        HCCL_DEBUG("[CcuAllReduceMeshMem2Mem1D] outputDataType is [INVALID], set outputDataType to[%d]", ctx.dataType);
    }
...
static CcuResult PairwiseLocalReduce(
    AllReduceMeshMem2Mem1DContext& ctx, ccu::LocalAddr myOutput, std::vector<ccu::LocalAddr>& inputVec,
...
static CcuResult CreateReduceLoopV1(
    AllReduceMeshMem2Mem1DContext& ctx, GroupReduceMem2MemVar& var, uint32_t size, HcclDataType dataType,
...
static CcuResult CreateReduceLoopV2(
    AllReduceMeshMem2Mem1DContext& ctx, GroupReduceMem2MemVar& var, uint32_t size, HcclDataType dataType,
    HcclDataType outputDataType, HcclReduceOp opType)
{
    constexpr uint32_t LOOP_NUM = 16;
    ...
    for (int32_t index = 0; index < MAX_LOOP_NUM; index++) { // 需要实例化2个Loop
        ...
        loops.body[index].reset(new ccu::Func([&ctx, index, bufBase, e0, e1, e2, size, &var, dataType, outputDataType,
                                               opType]() {
            for (uint32_t i = 0; i < size; i++) {
                if (i == ctx.arg->rankId) {
                    ccu::LocalCopy(ctx.moRes.ccuBuf[bufBase + i], var.loopSrc[index], var.loopLen[index], e0, 1 << i);
                } else {
                    ccu::LocalCopy(
...
static CcuResult ReduceLoopGroup(AllReduceMeshMem2Mem1DContext& ctx, ccu::LocalAddr srcOrg, CcuVersion ccuVersion)
{
    if (ccuVersion == CcuVersion::CCU_V1) {
        ...
        return ReduceLoopGroupV2(ctx, srcOrg);
    }
}
...
CcuResult ReduceLoopGroupV2(AllReduceMeshMem2Mem1DContext& ctx, ccu::LocalAddr srcOrg)
{
    ...
    CCU_CHK_RET(CreateReduceLoopV2(ctx, var, size, ctx.dataType, ctx.outputDataType, ctx.reduceOp));
    auto& loops = ctx.loopMap["reduce_mesh1d_mem2mem"];
...
static CcuResult BcastLocToRmt(
    AllReduceMeshMem2Mem1DContext& ctx, const ccu::Variable& srcAddr, const std::vector<ccu::Variable>& dstAddr)
{
    const auto* arg = ctx.arg;
    ...
    for (uint32_t rmtId = 0; rmtId < dstAddr.size(); rmtId++) {
        ...
        ccu::Write(
            arg->channels[channelIdx], ctx.remoteDstMem, ctx.srcMem, ctx.sliceSize, ctx.events[eventIdx],
...
static CcuResult DoLocalReduce(AllReduceMeshMem2Mem1DContext& ctx)
{
    ...
    if (arg->rankSize <= GROUP_REDUCE_MAX_PIECE_CNT) {
        ...
        CCU_CHK_RET(ReduceLoopGroup(ctx, srcLoc, GetCcuVersion()));
    } else {
...
static CcuResult ReduceRmtToLoc(
    AllReduceMeshMem2Mem1DContext& ctx, const std::vector<ccu::Variable>& srcAddr, const ccu::Variable& dstAddr)
{
    const auto* arg = ctx.arg;
    ...
    for (uint32_t rankIdx = 0; rankIdx < arg->rankSize; rankIdx++) {
        ...
        if (rankIdx == arg->rankId) {
            ...
            ccu::Read(
                arg->channels[channelIdx], ctx.reduceScatterDst[rankIdx], ctx.reduceScatterSrc[rankIdx], ctx.sliceSize,
...
CcuResult CcuAllReduceMeshMem2Mem1DKernel(CcuKernelArg arg)
{
    ...
    CCU_IF(ctx.mySliceSize != 0) { CCU_CHK_RET(DoRepeatAllReduce(ctx)); }
