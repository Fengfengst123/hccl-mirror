...
HcclResult CcuTempAllReduceMesh1DOneShot::CalcRes(
...
HcclResult CcuTempAllReduceMesh1DOneShot::KernelRun(
    const OpParam& param, const TemplateDataParams& templateDataParams, TemplateResource& templateResource)
{
    (void)param;
    buffInfo_ = templateDataParams.buffInfo;
    HCCL_INFO(
        "CcuTempAllReduceMesh1DOneShot KernelRun inputPtr(%p), inputSize(%d)", buffInfo_.inputPtr, buffInfo_.inputSize);
    ...
    CHK_RET(FillCachedArgs(
        submitInfo, inputAddr, outputAddr, token, goSize[0], goSize[1], goSize[2], goSize[3], buffInfo_.inBuffBaseOff,
...
HcclResult
CcuTempAllReduceMesh1DOneShot::FastLaunch(const OpParam& param, const TemplateFastLaunchCtx& tempFastLaunchCtx)
{
    (void)param;
    ...
    CcuResult launchRet = HcommCcuKernelLaunch(
        tempFastLaunchCtx.threads[0], tempFastLaunchCtx.ccuKernelSubmitInfos[0].kernelHandle,
