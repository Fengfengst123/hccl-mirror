...
{
    SetTemplateBuffInfo(
        tempAlgParamsReduceScatter, BufferType::INPUT, BufferType::HCCL_BUFFER, param.inputPtr, resCtx.cclMem.addr,
        resCtx.cclMem);

    SetTemplateBuffInfo(
        tempAlgParamsAllGather, BufferType::HCCL_BUFFER, BufferType::OUTPUT, resCtx.cclMem.addr, param.outputPtr,
        resCtx.cclMem);
    return;
}
