...
InsV2AllReduceSequence2DieExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1>::CalcCostCoeff(
    HcclComm comm, TopoInfoWithNetLayerDetails* topoInfo, const char* algName, const OpParam& param)
{
    ...
    std::vector<CostModelParam> params = [rankSize, rankSizeLevel0, portNumLevel0, netTypeLevel0, isPod] {
        ...
        auto p0 = InsAlgTemplate0::CalcCostCoeff(CalcCostCoeffParam{
            rankSizeLevel0 / 2, 1.0f / rankSizeLevel0, netTypeLevel0, BufferType::INPUT, BufferType::HCCL_BUFFER,
        ...
        auto p1 = InsAlgTemplate1::CalcCostCoeff(CalcCostCoeffParam{
            rankSizeLevel0 / 2, 1.0f / rankSizeLevel0, netTypeLevel0, BufferType::HCCL_BUFFER, BufferType::OUTPUT,
            BufferType::HCCL_BUFFER, portNumLevel0, isPod});
...
InsV2AllReduceSequence2DieExecutor<
    AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1>::~InsV2AllReduceSequence2DieExecutor()
{}
...
{
    // 初始化一些基本成员变量
    std::shared_ptr<InsAlgTemplate0> reduceTempAlg
        = std::make_shared<InsAlgTemplate0>(param, topoInfo->userRank, algHierarchyInfo.infos[0]);
    std::shared_ptr<InsAlgTemplate1> gatherTempAlg
        = std::make_shared<InsAlgTemplate1>(param, topoInfo->userRank, algHierarchyInfo.infos[0]);
    ...
    resourceRequest.notifyNumOnMainThread
        = std::max(resReqStepReduce.notifyNumOnMainThread, resReqStepGather.notifyNumOnMainThread);
    ...
    if (param.engine == COMM_ENGINE_CCU) {
        HCCL_INFO(
            "[InsV2AllReduceSequence2DieExecutor][CalcRes] intraTemplate has [%d] kernels.",
            resReqStepReduce.ccuKernelNum[0]);
        resourceRequest.ccuKernelInfos.insert(
            resourceRequest.ccuKernelInfos.end(), resReqStepReduce.ccuKernelInfos.begin(),
            resReqStepReduce.ccuKernelInfos.end());
        resourceRequest.ccuKernelNum.emplace_back(resReqStepReduce.ccuKernelNum[0]);
        HCCL_INFO(
            "[InsV2AllReduceSequence2DieExecutor][CalcRes] interTemplate has [%d] kernels.",
            resReqStepGather.ccuKernelNum[0]);
        std::for_each(
            resourceRequest.ccuKernelInfos.begin(), resourceRequest.ccuKernelInfos.end(), [](CcuKernelInfo& info) {
                info.resGroup = 0;
            });
        resourceRequest.ccuKernelInfos.insert(
            resourceRequest.ccuKernelInfos.end(), resReqStepGather.ccuKernelInfos.begin(),
            resReqStepGather.ccuKernelInfos.end());
        resourceRequest.ccuKernelNum.emplace_back(resReqStepGather.ccuKernelNum[0]);
        std::for_each(
            resourceRequest.ccuKernelInfos.begin() + resReqStepReduce.ccuKernelNum[0],
            resourceRequest.ccuKernelInfos.end(), [](CcuKernelInfo& info) {
                info.resGroup = 1;
            });
        HCCL_INFO(
...
template <typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1>
HcclResult InsV2AllReduceSequence2DieExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1>::Orchestrate(
    const OpParam& param, const AlgResourceCtxSerializable& resCtx)
{
    ...
    CHK_PRT_RET(
        ret != HCCL_SUCCESS,
        HCCL_ERROR(
            "[InsV2AllReduceSequence2DieExecutor][Orchestrate]errNo[0x%016llx] AllReduce executor kernel run failed",
            HCCL_ERROR_CODE(ret)),
        ret);
    return HCCL_SUCCESS;
}
...
template <typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1>
HcclResult InsV2AllReduceSequence2DieExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1>::CalcSliceInfoAllReduce(
    u64 dataCount)
{
...
template <typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1>
u64 InsV2AllReduceSequence2DieExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1>::RoundUp(
...
template <typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1>
HcclResult InsV2AllReduceSequence2DieExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1>::OrchestrateLoop(
    const OpParam& param, const AlgResourceCtxSerializable& resCtx)
{
    ...
    if (param.engine == COMM_ENGINE_CCU) {
        templateAlgRes0.ccuKernels.insert(
            templateAlgRes0.ccuKernels.end(), resCtx.ccuKernels.begin(),
            resCtx.ccuKernels.begin() + resCtx.ccuKernelNum[0]);
        templateAlgRes1.ccuKernels.insert(
            templateAlgRes1.ccuKernels.end(), resCtx.ccuKernels.begin() + resCtx.ccuKernelNum[0],
    ...
    CHK_PRT_RET(
        ...
        HCCL_ERROR("[InsV2AllReduceSequence2DieExecutor][OrchestrateOpbase] maxDataCountPerLoop is 0"),
        HCCL_E_INTERNAL);
    // 计算loopTimes
    ...
    for (u64 loop = 0; loop < loopTimes; loop++) {
        ...
        tempAlgParams0.inputSliceStride
            = (myRank_ == (rankSize_ - 1)) ? sliceInfoList_.at(myRank_ - 1).size : sliceInfoList_.at(myRank_).size;
        ...
        HCCL_DEBUG(
            "[InsV2AllReduceSequence2DieExecutor] testargs fortemplate0 tempAlgParams0.buffInfo.inBuffBaseOff[%u], "
            "tempAlgParams0.buffInfo.outBuffBaseOff[%u], tempAlgParams0.sliceSize[%u],"
            "tempAlgParams0.tailSize[%u], tempAlgParams0.inputSliceStride[%u], tempAlgParams0.outputSliceStride[%u].",
            tempAlgParams0.buffInfo.inBuffBaseOff, tempAlgParams0.buffInfo.outBuffBaseOff, tempAlgParams0.sliceSize,
            tempAlgParams0.tailSize, tempAlgParams0.inputSliceStride, tempAlgParams0.outputSliceStride);

        HCCL_DEBUG(
            "[InsV2AllReduceSequence2DieExecutor] testargs fortemplate1 tempAlgParams1.buffInfo.inBuffBaseOff[%u], "
            "tempAlgParams0.buffInfo.outBuffBaseOff[%u], tempAlgParams0.sliceSize[%u],"
            "tempAlgParams1.tailSize[%u], tempAlgParams1.inputSliceStride[%u], tempAlgParams1.outputSliceStride[%u].",
            tempAlgParams1.buffInfo.inBuffBaseOff, tempAlgParams1.buffInfo.outBuffBaseOff, tempAlgParams1.sliceSize,
            tempAlgParams1.tailSize, tempAlgParams1.inputSliceStride, tempAlgParams1.outputSliceStride);
    }
