    ...
    HcclDataType dataType, HcclComm comm, aclrtStream stream)
{
    ...
    CHK_RET(AllGatherVEntryLog(
        sendBuf, recvBuf, sendCount, recvCounts, recvDispls, dataType, stream, tag, rankSize, "HcclAllGatherV"));

    // 执行AllGatherV
    CHK_RET_AND_PRINT_IDE(
        AllGatherVOutPlace(sendBuf, recvBuf, sendCount, recvCounts, recvDispls, dataType, comm, stream, tag),
        tag.c_str());
    ...
    void* scratchMemAddr, uint64_t scratchMemSize)
{
    ...
    CHK_RET(AllGatherVEntryLog(
        sendBuf, recvBuf, sendCount, recvCounts, recvDispls, dataType, stream, opTag, rankSize,
        "HcclAllGatherVGraphMode", true));
    // 执行AllGatherV
    CHK_RET_AND_PRINT_IDE(
        AllGatherVOutPlaceGraphMode(
            sendBuf, recvBuf, sendCount, recvCounts, recvDispls, dataType, comm, stream, tag, resPack),
        opTag);
    ...
    uint64_t sendCount, void* recvBuf)
{
    ...
    RPT_INPUT_ERR(
        recvDispls == nullptr, "EI0003", std::vector<std::string>({"ccl_op", "value", "parameter", "expect"}),
    ...
    bool& allRecvCountsZero)
{
    ...
    if (std::all_of(recvCountsAddr, recvCountsAddr + rankSize, [](auto count) {
            return count == 0;
        })) {
    ...
    bool forceLog)
{
    if (forceLog || GetExternalInputHcclEnableEntryLog()) {
        ...
        HCCL_RUN_INFO(
            "Entry-%s: tag[%s], sendBuf[%p], recvBuf[%p], sendCount[%llu], dataType[%s], streamId[%d], deviceId[%d]",
            opName.c_str(), tag.c_str(), sendBuf, recvBuf, sendCount, GetDataTypeEnumStr(dataType).c_str(), streamId,
            deviceId);
    ...
    HcclDataType dataType, HcclComm comm, aclrtStream stream, const std::string& tag, const ResPackGraphMode& resPack)
{
    ...
    for (u64 i = 0; i < userRankSize; i++) {
        outputSize = (outputSize > (u64RecvDispls[i] + u64RecvCount[i]) * perDataSize) ?
                         outputSize :
                         (u64RecvDispls[i] + u64RecvCount[i]) * perDataSize;
    } // 结果为最大的displs加recvcount
    u64 varMemSize = (userRankSize + userRankSize) * sizeof(u64);
    void* paramMem = malloc(sizeof(OpParam) + varMemSize);
    if (!paramMem) {
        // 内存分配失败
        HCCL_ERROR("[AllGatherVOutPlaceGraphMode] malloc OpParam failed!");
        return HCCL_E_INTERNAL;
    }
    OpParam* tmpParamPtr = new (paramMem) OpParam();
    auto deleter = [](OpParam* p) {
