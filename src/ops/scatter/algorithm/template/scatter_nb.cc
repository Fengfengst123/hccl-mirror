...
namespace ops_hccl {
ScatterNB::ScatterNB() : AlgTemplateBase(), interRank_(0), interRankSize_(0) {}
...
HcclResult ScatterNB::RunScatterTx(const u32 step, std::vector<ChannelInfo>& channels)
{
    ...
    if (channelRight.protocol == COMM_PROTOCOL_ROCE) {
        ...
        CHK_PRT_RET(
            ret != HCCL_SUCCESS,
            HCCL_ERROR(
                "[Run][Scatter]rank[%u] step[%u] RightLink tx slices count [%u] Failed", interRank_, step, nSlices),
            ret);
    }
...
HcclResult ScatterNB::RunScatterRx(const u32 step, std::vector<ChannelInfo>& channels)
{
    ...
    // 为了避免在大数据量场景下触发网卡轮询机制，这里添加一组Data Notify，确保对端数据接收完成才进行下一次通信任务
    CHK_RET(
        static_cast<HcclResult>(HcommChannelNotifyRecordOnThread(thread_, channelLeft.handle, NOTIFY_IDX_DATA_SIGNAL)));
    return HCCL_SUCCESS;
}
...
// scatter的入口函数
HcclResult ScatterNB::RunAsync(const u32 rank, const u32 rankSize, std::vector<ChannelInfo>& channels)
{
