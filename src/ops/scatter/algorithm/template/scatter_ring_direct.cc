...
namespace ops_hccl {
ScatterRingDirect::ScatterRingDirect() : AlgTemplateBase() {}
...
HcclResult ScatterRingDirect::CheckParameters(const u32 rank, const u32 rankSize, std::vector<ChannelInfo>& channels)
{
...
HcclResult ScatterRingDirect::OneRankMemcpy()
{
    ...
    if (opInfo_->outputAddr != nullptr) {
        ...
        HCCL_DEBUG(
            "Memcpy operation: thread[main], rank[%u] starts to rcv offset[%llu], size[%llu] at userMemOut_", userRank_,
        ...
        HCCL_DEBUG(
            "Memcpy operation: thread[main], rank[%u] starts to rcv offset[%llu], size[%llu] at outputMem_", userRank_,
...
HcclResult
ScatterRingDirect::GetInitializedNeighborLinks(const u32 rank, const u32 rankSize, std::vector<ChannelInfo>& channels)
{
    // 收集左邻居信息
    ...
    const u32 stepsFromRank2Root, const u32 step, const Slice& txSlice, const Slice& rxSlice, const u32 rankSize)
{
    ...
    if (needReceive) {
        CHK_RET(
            static_cast<HcclResult>(HcommChannelNotifyRecordOnThread(thread_, leftChannel_.handle, NOTIFY_IDX_ACK)));
    }
    if (needSend) {
        CHK_RET(static_cast<HcclResult>(
            HcommChannelNotifyWaitOnThread(thread_, rightChannel_.handle, NOTIFY_IDX_ACK, CUSTOM_TIMEOUT)));
    }
    ...
    if (needSend) {
        // TxAsync
        CHK_RET(static_cast<HcclResult>(
            HcommChannelNotifyRecordOnThread(thread_, rightChannel_.handle, NOTIFY_IDX_DATA_SIGNAL)));
    }
    // 不同的rank会在不同的step开始持续发送操作，距离root节点越近，越早step开始发送操作
    if (needReceive) {
        void* dst = nullptr;
        if (step == rankSize - DMA_REDUCE_TWO_OFFSET && opInfo_->outputAddr != nullptr) {
            HCCL_DEBUG(
                "MemcpyAsync operation: step[%u] thread[main], dst rank[%u] starts to rcv offset[%llu], "
                "size[%llu] "
                "at userMemOut_",
        ...
        CHK_RET(static_cast<HcclResult>(
            HcommChannelNotifyWaitOnThread(thread_, leftChannel_.handle, NOTIFY_IDX_DATA_SIGNAL, CUSTOM_TIMEOUT)));
