...
HcclResult ScatterMesh::RunSendScatter(const u32 dstRank, const Slice& slice, std::vector<ChannelInfo>& channels)
{
    ...
    if (dstRank == interRank_) {
        if (inputMem_.addr != outputMem_.addr) {
            ...
            HCCL_DEBUG(
                "root rank copy from input[%p] range[%llu] to output[%p] range[%llu], size[%llu]", src,
        ...
        CHK_RET(static_cast<HcclResult>(
            HcommChannelNotifyWaitOnThread(thread_, channels[dstRank].handle, NOTIFY_IDX_ACK, CUSTOM_TIMEOUT)));
        CHK_RET(static_cast<HcclResult>(
            HcommChannelNotifyRecordOnThread(thread_, channels[dstRank].handle, NOTIFY_IDX_DATA_SIGNAL)));
    }
    return HCCL_SUCCESS;
}
...
HcclResult ScatterMesh::RunRecvScatter(const u32 srcRank, const Slice& slice, std::vector<ChannelInfo>& channels)
{
    ...
    CHK_RET(
        static_cast<HcclResult>(HcommChannelNotifyRecordOnThread(thread_, channels[srcRank].handle, NOTIFY_IDX_ACK)));
    CHK_RET(static_cast<HcclResult>(
        HcommChannelNotifyWaitOnThread(thread_, channels[srcRank].handle, NOTIFY_IDX_DATA_SIGNAL, CUSTOM_TIMEOUT)));
